...
@section('breadcrumb', 'Home / Broadcast / Edit')
@section('header', 'Edit Broadcast')
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-500 px-6 py-5">
            <div class="flex items-center gap-3">
                ...
                <div><h2 class="text-lg font-semibold text-white">Edit Broadcast</h2><p class="text-sm text-white/80">Perbarui broadcast</p></div>
            </div>
        </div>
        <form method="post" action="{{ route('admin.broadcasts.update', $broadcast) }}" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf @method('PUT')
            <div>
                ...
                <div class="grid grid-cols-1 gap-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4" required class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:ring-2">{{ old('description', $broadcast->description) }}</textarea>
                    </div>
                </div>
            </div>
