    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        <div class="bg-gradient-to-r from-emerald-600 to-emerald-500 px-6 py-5">
            <div class="flex items-center gap-3">
                ...
                <div><h2 class="text-lg font-semibold text-white">Edit Status</h2><p class="text-sm text-white/80">Kode: {{ $status->code }}</p></div>
            </div>
        </div>
        <form method="post" action="{{ route('admin.statuses.update', $status) }}" class="p-6 space-y-6">
            @csrf @method('PUT')
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input name="code" value="{{ old('code', $status->code) }}" required class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:ring-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Name <span class="text-red-500">*</span></label>
                        <input name="name" value="{{ old('name', $status->name) }}" required class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:ring-2">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-1.5">Sort Order</label>
                        <input type="number" name="sort_order" value="{{ old('sort_order', $status->sort_order) }}" min="0" class="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-sky-500 focus:ring-2">
                    </div>
                </div>
            </div>
